...
def _drop_degenerate_inners(shape):
    ...
    for inner in shape.interiors:
        # need to make a polygon of the linearring to get the _filled_ area of the closed ring.
...
def _polytree_node_to_shapely(node):
    ...
    if node.IsHole:
        ...
        # This is because children of holes must be outers, and should be on the polygons list.
        ...
        # if they cause the polygon to become invalid. If the shape has lots of inners, then this can mean a
        # proportional amount of work, and may take 1,000s of seconds. Instead, we can group inners together, which
        # reduces the number of times we call the expensive 'difference' method.
        block_size = 200
        ...
        for inner in inners:
            ...
            # )
            #
            # NOTE: this throws away the inner ring if we can't produce a valid difference. Not ideal,
            # but we'd rather produce something that's valid than nothing.
            if diff.is_valid:
                poly = diff
...
def _polytree_to_shapely(tree):
    ...
    # expect no leftover children --- should all be incorporated into polygons by the time recursion returns to the
    # root.
    assert len(children) == 0
...
def make_valid_polygon(shape):
    ...
    degeneracy. This process attempts to make a polygon valid while retaining as much of its extent or area as
    possible.
    ...
    pyclipper does not consider these to be invalid. Therefore, we follow by using the `buffer(0)` technique to attempt
